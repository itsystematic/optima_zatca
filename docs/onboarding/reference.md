...
| **Sourced from Company** | `organization_identifier` and `organization_name` — the CSR takes `Company.tax_id` and `Company.company_name_in_arabic` directly, not these fields; the stored values feed the invoice payload's `CompanyID` / `RegistrationName` |
