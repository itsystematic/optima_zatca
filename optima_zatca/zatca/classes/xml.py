...
class ZatcaXmlGenerator:
    ...
    def _add_allowance_charges(self):
        ...
        # if sales invoice one tax category like(S) and has discount on Document level
        # then the get the discount from the invoice directly(discount_amount)
        has_single_allawance = ( # 
            len(self.sales_invoice.get("TaxSubtotal" , [])) == 1 and
            self.sales_invoice.get("TaxSubtotal")[0].get("AllowanceChargeAmount" , 0.00) > 0
        )
        ...
        for charge in self.sales_invoice.get("TaxSubtotal", []):
            ...
            allowance_elem = self._create_allowance_charge_element(charge,has_single_allawance)
            allowance_elem.tail = "\n    "
            etree.indent(allowance_elem, space="    ", level=1)
            parent.insert(insert_position, allowance_elem)
            insert_position += 1
    
    def _create_allowance_charge_element(self, charge: Dict, has_single_allawance: bool) -> etree.Element:
