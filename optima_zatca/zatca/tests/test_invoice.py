...
class TestSubmitToZatcaApi(InvoiceTestCase):
    def test_submit_to_zatca_api_calls_make_invoice_request_with_expected_payload(self):
        ...
        invoice.company_settings = {"authorization": "Bearer token"}
        invoice.xml.hash = "abc123"

        with patch.object(invoice_module, "make_invoice_request") as mock_request:
            mock_request.return_value = MagicMock(status_code=200)

            result = _submit_to_zatca_api(invoice, "encoded_xml")
...
class TestSuccessfulResponseHelpers(InvoiceTestCase):
    ...
    def test_get_response_qrcode_returns_empty_string_when_response_failed(self):
        result = _get_response_qrcode(MagicMock(status_code=400), MagicMock(), False)
    ...
    def test_get_response_qrcode_reads_qr_from_cleared_invoice_when_successful(self):
        ...
        with patch.object(
            ...
            result = _get_response_qrcode(response, invoice, True)
...
class TestLogAction(InvoiceTestCase):
    def setUp(self):
        ...
        self.invoice.xml.hash = "hash"
        self.invoice.xml.qr_code = "qr"
    ...
    def test_log_action_uses_formatted_response_when_json_is_available(self):
        ...
        with (
            ...
            patch.object(invoice_module, "serialize_invoice_xml", return_value=b"<xml/>"),
...
class TestHandlePostSuccess(InvoiceTestCase):
    def test_handle_post_success_skips_on_failure(self):
        with patch.object(
            ...
            _handle_post_success(MagicMock(), MagicMock(), False)
    ...
    def test_handle_post_success_creates_prepayment_for_non_normal_invoice(self):
        ...
        invoice = MagicMock()
        invoice.zatca_invoice = {"UUID": "uuid-123"}
        ...
        with (
            ...
            _handle_post_success(sales_invoice, invoice, True)
    ...
    def test_handle_post_success_auto_submits_when_flag_is_false(self):
        ...
        invoice = MagicMock()
        invoice.zatca_invoice = {"UUID": "uuid"}
    ...
    def test_handle_post_success_skips_submit_when_manual_flag_is_true(self):
        ...
        with (
            ...
            _handle_post_success(sales_invoice, MagicMock(), True)
...
class TestSendToZatcaOrchestrator(InvoiceTestCase):
    def test_send_to_zatca_orchestrates_all_helpers(self):
        ...
        invoice = MagicMock()
        invoice.xml = MagicMock()
        sales_invoice = MagicMock()

        with (
            patch.object(invoice_module.frappe, "get_doc", return_value=sales_invoice) as mock_get_doc,
            patch.object(invoice_module, "ZatcaInvoiceData", return_value=invoice) as mock_zatca_cls,
            patch.object(invoice_module, "_validate_before_send", return_value=True) as mock_validate,
            patch.object(invoice_module, "encode_invoice_xml_for_api", return_value="encoded") as mock_encode,
        ...
        mock_encode.assert_called_once_with(invoice.xml)
        mock_submit.assert_called_once_with(invoice, "encoded")
        mock_success.assert_called_once_with(response)
        mock_qr.assert_called_once_with(response, invoice, True)
        ...
        mock_post.assert_called_once_with(sales_invoice, invoice, True)
    ...
    def test_send_to_zatca_returns_false_when_validation_fails(self):
        ...
        with (
            ...
            patch.object(invoice_module, "encode_invoice_xml_for_api") as mock_encode,
        ):
            result = send_to_zatca("SINV-0001")
