...
class TestLogAction(InvoiceTestCase):
    ...
    def test_log_action_uses_formatted_response_when_json_is_available(self):
        ...
        with (
            patch.object(workflow_module, "make_action_log") as mock_log,
            patch.object(workflow_module, "format_zatca_response", return_value="formatted"),
        ):
            _log_action(
                self.sales_invoice,
                self.log_context,
...
class TestSubmitInvoiceIfAutoSubmitEnabled(InvoiceTestCase):
    def test_auto_submits_when_flag_is_false(self):
        ...
        with (
            patch.object(workflow_module.frappe.db, "get_single_value", return_value=0),
...
class TestSubmissionWorkflow(InvoiceTestCase):
    def test_submit_sales_invoice_to_zatca_orchestrates_all_helpers(self):
        ...
        with (
            ...
            patch.object(workflow_module, "_is_successful_response", return_value=True) as mock_success,
            patch.object(workflow_module, "_get_response_qrcode", return_value="qrcode") as mock_qr,
            patch.object(workflow_module, "_update_invoice_document") as mock_update,
            patch.object(workflow_module, "_log_action") as mock_log,
            patch.object(workflow_module, "_handle_post_success") as mock_post,
        ):
